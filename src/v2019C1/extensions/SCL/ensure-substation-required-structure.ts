...
export function ensureSubstationRequiredStructure(params: Scl.MethodsParams<'SCL'>) {
	...
	>(params: {
		...
		const { focusLevel, names = {} } = params
		const { substation: substationName, voltageLevel: voltageLevelName, bay: bayName } = names

		const initialChain = chain({
			contextPromise: Promise.resolve(contextPromise),
		})
		// Find existing Substation
		const { Substation: substations = [] } = await initialChain.findDescendants({
			tagName: 'Substation',
		...
		if (!substations?.[0]) {
			...
			substationChain = initialChain.addChild({
				id: substationId,
				tagName: 'Substation',
			...
			substationChain = initialChain.goToElement({ tagName: 'Substation', id: substationId })
		}
