...
const behaviorRef = { tagName: 'BehaviorDescription', id: 'bd-s' } as Scl.Ref<'BehaviorDescription'>
...
// A floating BehaviorDescription (cross-cutting satellite) whose InputVar points
// at the function's LNode.
const sourceXml = /* xml */ `
	<SCL ${ns} ${id}="fsd">
...
const mutate = async (tx: Scl.Transaction): Promise<void> => {
	...
	await tx.update(behaviorRef, { attributes: { desc: 'updated behavior' } })
}

describe('lifecycle.apply — carried BehaviorDescription (cross-cutting satellite)', () => {
	const testCases: SclTest.TestCases<TestCase> = {
		'accepting the function group also updates the BehaviorDescription that references it': {
			sourceXml,
			targetXml,
			mutate,
			decide: () => new Map(),
			expectedQueries: [
				...
				'//v2019C1:BehaviorDescription[@templateUuid="bd-src-uuid"][@desc="updated behavior"]',
			],
		},

		'skipping the function group leaves the BehaviorDescription untouched': {
			sourceXml,
			targetXml,
			mutate,
			decide: skipAll(),
			unexpectedQueries: [
				'//default:Function[@name="Prot"][@desc="updated function"]',
				'//v2019C1:BehaviorDescription[@desc="updated behavior"]',
			],
		},
	}
